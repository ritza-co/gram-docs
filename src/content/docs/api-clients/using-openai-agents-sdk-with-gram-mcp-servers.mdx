...
OpenAI provides two main approaches for integrating with MCP servers:
...
If you're unsure which approach fits your needs, start with the [Responses API guide](/api-clients/using-openai-api-with-gram-mcp-servers) for simpler implementations.
...
export MCP_TASKMASTER_OAUTH_TOKEN_ACCESS_TOKEN=your-taskmaster-oauth-token
```
...
ACCESS_TOKEN = os.getenv("MCP_TASKMASTER_OAUTH_TOKEN_ACCESS_TOKEN")

if not API_KEY or not ACCESS_TOKEN:
    raise ValueError("Missing TaskMaster environment variables")
...
taskmaster_tool = HostedMCPTool(
    tool_config={
        ...
        "headers": {
            ...
            "MCP-TASKMASTER-OAUTH-TOKEN-ACCESS-TOKEN": ACCESS_TOKEN
        },
        "require_approval": "never"
    }
)
    ...
    taskmaster_tool = HostedMCPTool(
        tool_config={
            ...
            "headers": {
                ...
                "MCP-TASKMASTER-OAUTH-TOKEN-ACCESS-TOKEN": os.getenv('MCP_TASKMASTER_OAUTH_TOKEN_ACCESS_TOKEN')
            },
            "require_approval": "never"
        }
    )
...
taskmaster_tool = HostedMCPTool(
    tool_config={
        ...
        "server_url": "https://app.getgram.ai/mcp/your-taskmaster-slug",
        "allowed_tools": [
            # Example tool names - replace with actual tool names from your MCP server
            "your_server_list_tasks",
            "your_server_create_task",
            "your_server_update_task",
            "your_server_get_project"
...
## Comparing with Responses API

Here's how the same task management scenario differs between approaches:
...
response2 = client.responses.create(
    model="gpt-4.1",
    tools=[{"type": "mcp", "server_url": "..."}],
    input="Add a task to the Website Redesign project: 'Design homepage'"
)
```
...
result1 = Runner.run_sync(agent, "Create a project called 'Website Redesign'")
result2 = Runner.run_sync(agent, "Add a task to that project: 'Design homepage'")
```
