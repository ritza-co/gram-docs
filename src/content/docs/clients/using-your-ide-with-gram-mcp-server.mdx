...
Before connecting your IDE to a Taskmaster MCP server, you'll need to create one first. Follow the [Creating a Taskmaster MCP server](/build-mcp/creating-taskmaster-mcp-server) guide to set up your server.

Once you have your Taskmaster MCP server ready, you can connect it to your IDE. The following sections show you how to add the MCP server configuration to the most popular IDE-based MCP clients.
...
### Step 2: (Optional) Configure authentication

If you're using an **authenticated server**, you can add the authentication details to the `settings.json` file using the configuration from your Taskmaster creation guide.
...
- **Environment Variables:** Use the environment variables from your Taskmaster MCP server configuration (if using an authenticated server)
