...
Add your Taskmaster MCP server configuration to the file. For a public server, it looks like this:
...
```json
{
  "mcpServers": {
    "GramTaskmaster": {
      ...
      "args": [
          "mcp-remote",
          "https://app.getgram.ai/mcp/<your-slug>",
          "--header",
...
Replace `your-taskmaster-slug` with your actual server slug.
...
Claude Connectors allow you to connect Claude Desktop to your Gram-hosted MCP server without manually editing the configuration file. This is more user-friendly, but comes with one important limitation: **your MCP server must be public** and must not require environment variables.
 ...
 For this section we'll use the [Push Advisor example](https://github.com/ritza-co/gram-examples/tree/main/push-advisor-api) MCP server.
...
```text
Check if I can push to prod today.
```
