...
To add an MCP server in Claude Code  use the `claude mcp add` command.
...
Therefore, you can run the following command in your terminal using your own MCP server URL to add the server:
...
Replace `<your-api-key>` with the actual values from your Taskmaster MCP server configuration, and replace `your-taskmaster-slug` with your actual MCP server slug.

If you're using the **Managed Authentication** configuration, run the following command:
...
Replace `<your-gram-api-key>` with your actual Gram API key and `your-taskmaster-slug` with your actual MCP server slug.
...
- `--scope project` adds the server to a local `.mcp.json` file that can be shared with other users on the project. You can [read more about other scope options](https://docs.anthropic.com/en/docs/claude-code/mcp#mcp-installation-scopes) at Anthropic's documentation.
